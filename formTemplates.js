// Generador de HTML para el formulario de perfil
...
// Generador de HTML para el formulario de privacidad
export function generatePrivacyForm(userData) {
  return `
    <h2 class="text-xl font-semibold mb-4">Privacidad</h2>
    <form id="privacyForm" class="space-y-4">
      <div><label class="block font-semibold">Correo Electrónico:</label><input type="email" id="privacyEmail" value="${userData.EMAIL}" class="w-full p-2 border rounded" disabled></div>
      <div><label class="block font-semibold">Contraseña:</label><input type="password" id="privacyPassword" value="${userData.PASSWORD}" class="w-full p-2 border rounded" disabled></div>

      <div class="flex space-x-4">
        <button type="button" id="privacyEditBtn" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Editar</button>
        <button type="submit" id="privacySaveBtn" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 hidden">Guardar Cambios</button>
      </div>
    </form>
